...
const SEARXNG_MAX_RESULTS = Math.max(
  10,
  Math.min(100, parseInt(process.env.SEARXNG_MAX_RESULTS || '70', 10))
)
...
export async function POST(request: Request) {
  ...
  const SEARXNG_DEFAULT_DEPTH = process.env.SEARXNG_DEFAULT_DEPTH || 'advanced'
...
async function advancedSearchXNGSearch(
  ...
  searchDepth: 'advanced' | 'advanced' = 'advanced',
  ...
  const SEARXNG_ENGINES =
    process.env.SEARXNG_ENGINES || 'google scholar,bing,duckduckgo,Scopus,sciencedirect,JSTOR,PubMed,arXiv,mwmbl,startpage,stract,swisscows,qwant'
  const SEARXNG_TIME_RANGE = process.env.SEARXNG_TIME_RANGE || 'None'
  const SEARXNG_SAFESEARCH = process.env.SEARXNG_SAFESEARCH || '2'
